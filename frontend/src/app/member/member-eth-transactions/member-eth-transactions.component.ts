...
@Component({
  selector: 'app-member-eth-transactions',
  templateUrl: './member-eth-transactions.component.html',
  styleUrls: ['./member-eth-transactions.component.scss']
})
export class MemberEthTransactionsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.get_btc_transaction_url   = this.rest.api_url + "/api/member/get_eth_transaction";
  ...
  cancelTransaction(t_id)
  {
    ...
    this.http.post(this.cancel_transaction_url, this._param).subscribe(response=>
      {
        this.loadTable();
      },
      error=>
  ...
  loadTable()
  {
    ...
    this.http.post(this.get_btc_transaction_url,this._param).subscribe(response=>
      {
